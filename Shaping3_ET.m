...
num_pass = 8;            % Number of good clicks to end block
...
% fileNum = [num2str(date) '_' num2str(subject)];

% EyelinkFilename = [num2str(date) '_' num2str(subject) '_' num2str(ID)];
% EyelinkFilename = [num2str(date)];
edfFile = [num2str(ID) '.edf'];
% EyelinkFilename = '0925';
%%  PREPARE
...
%     EYELINK('command', 'link_sample_data = RIGHT,LEFT,GAZE,AREA');
    ...
    Eyelink('openfile',[num2str(ID) '.edf']);
...
for i = 1:num_trial   % Number of trials
    ...
    if (i == round(num_trial/3))|| (i == round(num_trial/3)*2)
        EyelinkDoDriftCorrection(el);
    end
        
        Eyelink('Message', 'Trial begins');
...
for j = 1:50    % Number of clicks until reaching threshold 15 times overall
...
%    WaitSecs(0.1); 
   ...
   if (Count >= num_pass)|| (max(diff([0 (find(~(Target_click{i,1} > 0))') numel(Target_click{i,1})' + 1]) - 1) >= 15)
      WaitSecs(0.5);
