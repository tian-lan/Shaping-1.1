...
% Screen setup
[w1,winRect]=Screen('OpenWindow',0,[0 0 0]);
...
Screen('FillOval', w1, grey, Dot_Rect);
...
Screen('FillPoly', w1, white, frame_start ,1);
Screen('FillPoly', w1, white, frame_end ,1);

% WaitSecs(0.5);

%%%%%%%%%%%%Display good and bad clicks
    if Yes{n,1}(m,1) == 1
    Screen('DrawDots', w1, [Click{n,1}(m,1);Click{n,1}(m,2)], 10, green, [0 0], 1);
    else
    Screen('DrawDots', w1, [Click{n,1}(m,1);Click{n,1}(m,2)], 10, red, [0 0], 1);
    end
...
Screen('FrameOval', w1, grey, Target_Rect(1:4,m), 2);


DrawFormattedText(w1, num2str(m), 0.9*winRect(3), 0.1*winRect(4), white);
...
for t = 1:size(Click{n,1},1) - 1
    Screen('DrawLines', w1, [Click{n,1}(t,1) Click{n,1}(t+1,1);Click{n,1}(t,2) Click{n,1}(t+1,2)], 2, white, [0 0]);
end  % t

DrawFormattedText(w1, num2str(size(Click{n,1},1)), 0.9*winRect(3), 0.1*winRect(4), white);
...
while press
    ...
    pressedKey1 = KbName(keyCode) ;
    if  strcmpi(pressedKey1, 'space') == 1
        press = false;
    end
